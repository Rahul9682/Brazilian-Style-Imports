...
private var kDLHamburguerInitialContainerViewWidth: CGFloat = 250

/**
 * This class contains the container view for the hamburguer main elements: the menu and the content.
 */
class HamburguerContainerVC: UIViewController {
    // structure
    weak var hamburguerViewController: HamburguerVC!      // root hamburguer view controller
...
//        }
        ...
        let gestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(HamburguerContainerVC.tapGestureRecognized(_:)))
        ...
        if self.hamburguerViewController.menuViewController != nil {
            self.addChild(self.hamburguerViewController.menuViewController)
            ...
            self.hamburguerViewController.menuViewController.didMove(toParent: self)
        }
    ...
    func setContainerFrame(_ frame: CGRect) {
        ...
        let shadowFrame = CGRect(x: -768, y: y, width: w, height: h)
